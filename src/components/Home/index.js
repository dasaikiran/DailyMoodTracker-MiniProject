...
class Home extends Component {
  ...
  onDateLiClick = (id, month) => {
    const {calenderList, activeEmoji} = this.state
    const datesList = calenderList[month - 1].dates
    const liObj = datesList.find(item => item.id === id)
    const actEmojiObj = emojisList.find(item => item.id === activeEmoji)
    let newObj
  ...
  render() {
    ...
    console.log(activeEmojiName, activeDay)
    console.log(month)
    return (
      <>
        <Header />
        <div className="home-body-container">
              ...
              {emojisList.map(item => {
                ...
                return (
                  <li key={id} className="emoji-li">
                    ...
                    >
                      <p className={`emoji-para ${activeEmojiPara}`}>
                        {emojiName}
                      </p>
                      <img
          ...
          <div className="calender-container">
            <Calender
              ...
              onDateLiClick={this.onDateLiClick}
